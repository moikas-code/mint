...
export function SdkConnectionProvider({
  connector,
  children,
}: React.PropsWithChildren<ISdkConnectionProviderProps>) {
  const {environment} = useContext(EnvironmentContext);
  const conn = useRxOrThrow(connector.connection);
  const sdk =
    conn.status === 'connected'
      ? createRaribleSdk(conn.connection.wallet, environment)
      : undefined;
  ...
  const context: IConnectorContext = {
    ...
    walletAddress:
      conn.status === 'connected'
        ? conn.connection.blockchain + ':' + conn.connection.address
        : undefined,
  };
